...
def parse_args():
    ...
    ### EMOTION CONFIG ###
    parser.add_argument("--use_emotions", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--emotion_embed_dim", type=int, default=64)
...
### Get num_emotions from metadata if using emotions ###
num_emotions = 0
if args.use_emotions:
    import pandas as pd
    try:
        train_meta = pd.read_csv(args.path_to_train_manifest)
        if 'emotion' in train_meta.columns:
            num_emotions = len(train_meta['emotion'].unique())
            accelerator.print(f"Found {num_emotions} unique emotions in dataset")
        else:
            accelerator.print("Warning: use_emotions=True but 'emotion' column not found in metadata. Setting num_emotions=0")
    except Exception as e:
        accelerator.print(f"Warning: Could not read metadata to get num_emotions: {e}. Setting num_emotions=0")
...
# Get emotion mapping from trainset to use for testset
emotion_to_id = trainset.emotion_to_id if args.use_emotions else None
# Verify num_emotions matches what we found in metadata
if args.use_emotions:
    actual_num_emotions = trainset.num_emotions
    if actual_num_emotions != num_emotions:
        accelerator.print(f"Warning: num_emotions mismatch. Metadata had {num_emotions}, dataset has {actual_num_emotions}. Using dataset value.")
        num_emotions = actual_num_emotions
...
collator = TTSCollator(use_emotions=args.use_emotions)
...
for epoch in range(completed_epochs, args.training_epochs):
    ...
    for batch in trainloader:
        ...
        if args.use_emotions:
            texts, text_lens, mels, stops, encoder_mask, decoder_mask, emotions = batch
            emotions = emotions.to(accelerator.device)
        else:
            texts, text_lens, mels, stops, encoder_mask, decoder_mask = batch
            emotions = None
        ...
        mels_out, mels_postnet_out, stop_preds, _ = model(
            texts, text_lens.to("cpu"), mels, encoder_mask, decoder_mask, emotion_ids=emotions
        )
